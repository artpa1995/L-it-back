...
<div class="container">
  <div class="row ">
    ...
    <h1>add iqons</h1>
    <form method="post" action="{{ route('addPageIqons', [4,'contact']) }}" enctype="multipart/form-data">
      @csrf
            ...
            @foreach($pageIatems as $data)
                @if(isset($data->id) && !isset($data->lang))
                    ...
                    <div id="page_items{{$data->id}}" class="collapse" style="padding:10px;">
                        <form method="post" action="{{ route('updatePageIqon', [$data->id,$data->page_id,'contact']) }}" enctype="multipart/form-data" style="border:3px solid grey; padding:10px; box-sizing: border-box;">
                            @csrf
                            <input type="file" class="" name="image">
                            ...
                            <a href="{{ route('removePageItem', [$data->key,'contact']) }}" class="btn btn-danger">Remove</a>
                        </form>
                    </div>
    ...
    <br>
    <h1>add texts</h1>
    <form method="post" action="{{ route('setPageItem', [4,'contact']) }}">
      @csrf
        ...
        <div class="row page_blocks">
          @foreach($pageIatems as $data)
            @if(isset($data->id) && isset($data->lang))
                ...
                <div id="page_items{{$data->id}}" class="collapse" style="padding:10px;">
                  <form method="post" action="{{ route('updatePageItem', [$data->id,$data->page_id,$data->lang,'contact']) }}" enctype="multipart/form-data" style="border:3px solid grey; padding:10px; box-sizing: border-box;">
                      @csrf
                      <p>{{$data->lang}}</p>
                      <label for="">Key:</label>
                      <input type="text" name="key" autofocus value="{{$data->key}}">
  ...
  $(document).ready(function(){
    ...
    $('#lang2').on('change',function(){
      ...
      $.ajax({
            ...
            type:"post",
            datatType : 'json',
            data: {"lang" : lang, "_token": "{{ csrf_token() }}", 'id' : 4},
